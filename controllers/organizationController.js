...
const getExactSearch = async(CompanyName) => {
    try {
        ...
        //     regexArr.push(regex)
        // })
        const searchResults = await Organization.find({ "CompanyName": { "$regex": CompanyName, "$options": "i" } });
        return searchResults;
    } catch {
        console.log("error in exact match")
        return [];
    }
};
...
const getFuzzyResults = async(CompanyName) => {
    try {
        const query = {
            index: "CompanyName",
            compound: {
                should: CompanyName.split(/\W+/).map((word) => ({
                    text: {
                        query: word,
                        path: {
                            wildcard: '*'
...
const getRegexResults = async(CompanyName) => {
    try {
        const subQueries = CompanyName.split(/\W+/).map((word) => ({
            CompanyName: {
                $regex: word,
                $options: "i",
            }
...
const checkSame = async(array1, array2) => {
    console.log(array1, array2)

    const intersection = array1.filter(value => array2.includes(value));

    if (array1.length == intersection.length) {
        return true;
    }
...
const isValidResults = async(searchResults, CompanyName) => {
    var validRes = []
    const terms = CompanyName.split(/\W+/);

    for (var i = 0; i < searchResults.length; i++) {
        var resultantName = searchResults[i].CompanyName.toLowerCase().split(/\W+/)
        const isSame = await checkSame(terms, resultantName);
        if (isSame) {
            validRes.push(searchResults[i]);
        }
    }
...
exports.getAllOrganization = async(req, res) => {
    ...
    console.log("Calling Exact match");
    const exactMatchResults = await getExactSearch(CompanyName);
    if (exactMatchResults.length > 0) {
        searchResults.push(...exactMatchResults);
        for (var i = 0; i < exactMatchResults.length; i++) {
            var currSlug = exactMatchResults[i].slug;
            hashMap[currSlug] = 1;
        }
    }
    ...
    if (fuzzyResults.length > 0) {
        ...
        for (var i = 0; i < fuzzyResults.length; i++) {
            var currSlug = fuzzyResults[i].slug;
            if (currSlug in hashMap) {

            } else {
                hashMap[currSlug] = 1;
                searchResults.push(fuzzyResults[i]);
            }
        }
    }
    ...
    if (regexResults.length > 0) {
        for (var i = 0; i < regexResults.length; i++) {
            var currSlug = regexResults[i].slug;
            // console.log(hashMap[currSlug])
            if (currSlug in hashMap) {

            } else {
                hashMap[currSlug] = 1;
                searchResults.push(regexResults[i]);
            }
        }
    ...
    if (!skipExternalFetch) {
        ...
        if (resultsFromGoogle.length > 0) {
            searchResults = insert(searchResults, 0, ...resultsFromGoogle)
        }
    }
